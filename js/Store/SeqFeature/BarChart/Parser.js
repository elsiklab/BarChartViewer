/* The function to parse the BarChart files. The BarChart file format (BED-6+3) is "chr\tstart(0based)\tEnd(1based)\tname\tscore\tstrand\taltname\tbarcount\tvallist
...
var bed_feature_names = 'seq_id start end name score strand alt_name exp_count exp_scores cell_types'.split(" ");
...
return declare( null, {
	...
	if( parsed.exp_scores != null )
	    parsed.exp_scores = parsed.exp_scores.split(',').map(Number);

        if( parsed.cell_types != null )
            parsed.cell_types = parsed.cell_types.split(',')
